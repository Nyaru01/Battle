...
func _profile() -> void:
	...
	for side in [BattleSim.PLAYER, BattleSim.ENEMY]:
		for index in range(12):
			...
			simulation.play_card(side, card_id, index % 2)
	...
	print("Visual profile: %d rigs | %d frames | %.1f FPS average | %.2f ms/frame" % [simulation.units.size(), SAMPLE_FRAMES, average_fps, duration_seconds * 1000.0 / SAMPLE_FRAMES])
