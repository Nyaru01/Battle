...
func _test_android_export_configuration() -> void:
	...
	_expect(config.count("version/code=58") == 2 and config.count("version/name=\"0.58.0-alpha\"") == 2, "both Android presets expose the v0.58 package version")
...
func _test_visual_system() -> void:
	...
	_expect(ResourceLoader.exists("res://assets/v057/ui/button-forged-blue-v057.png"), "the original forged blue v0.57 action button is packaged")
	_expect(ResourceLoader.exists("res://assets/v058/ui/button-forged-compact-v058.png") and ResourceLoader.exists("res://assets/v058/ui/button-forged-square-v058.png"), "compact and square forged v0.58 controls are packaged")
	_expect(ArenaTheme.fantasy_action() is StyleBoxTexture, "the primary home action uses a real sliced texture instead of a flat drawn rectangle")
	_expect(ArenaTheme.nav_button(true) is StyleBoxTexture, "home navigation uses the compact forged texture family")
	var wide_button := Button.new()
	wide_button.custom_minimum_size = Vector2(320.0, 60.0)
	ArenaTheme.apply_button(wide_button, "primary", 18)
	_expect((wide_button.get_theme_stylebox("normal") as StyleBoxTexture).texture == ArenaTheme.FORGED_BUTTON_COMPACT, "wide menu actions automatically receive the compact forged plaque")
	var icon_button := Button.new()
	icon_button.custom_minimum_size = Vector2(58.0, 54.0)
	ArenaTheme.apply_button(icon_button, "secondary", 24)
	_expect((icon_button.get_theme_stylebox("normal") as StyleBoxTexture).texture == ArenaTheme.FORGED_BUTTON_SQUARE, "small menu commands automatically receive the square forged frame")
	wide_button.free()
	icon_button.free()
...
func _test_v055_home_lobby() -> void:
	...
	_expect(scene.ui_root.find_child("ArenaTag", true, false) == null and scene.ui_root.find_children("*", "FantasyFrame", true, false).size() == 2, "home replaces the floating arena tag with two cohesive sculpted HUD frames")
	...
	_expect(scene.primary_action_button.get_theme_stylebox("normal") is StyleBoxTexture, "Combat presents the forged blue textured control")
	...
	_expect((navigation.get_child(0) as Button).get_theme_stylebox("normal") is StyleBoxTexture, "every home navigation destination uses a textured forged control")
